...
public abstract class SoapService {
	 ...
	 * @author Justin Phlegar
	 * @version Created: 08/28/2014
	 * @param doc Document XML file of the Request to be stored in memory
	 */
	 ...
	 * @author Justin Phlegar
	 * @version Created 08/28/2014
	 * @param doc Document: XML file of the Response to be stored in memory
	 */
	 ...
	 * @author Justin Phlegar
	 * @version Created: 08/28/2014
	 * @param url String: URL Endpoint of the Service Under Test
	 */
	 ...
	 * @author Justin Phlegar
	 * @version Created: 08/28/2014
	 * @param url String: Service Name of the Service Under Test
	 */
	 ...
	 * @author Justin Phlegar
	 * @version Created: 08/28/2014
	 * @param url String: Operation Name of the Service Under Test
	 */
	 ...
	 * @param version SOAPConstants.SOAP_1_1_PROTOCOL or
	...
	/**
	 * Find and open the excel or csv file sent. If successful, look and find the
	 * matching scenario name then return its xpath and value data.
	 *
	 * @author Justin Phlegar
	 * @version Created: 08/28/2014
	 * @param file     String: File location either in the project or the hard disk
	 *                 path
	 * @param scenario String: Name of the scenario to poll data for
	 */
	protected Object[][] getTestScenario(String filePath, String scenario) {
		...
		for (int i = 1; i < endRow; i++, ci++) {
			...
			for (int j = startCol + 1; j < endCol; j++, cj++) {
				...
				// System.out.println( "Tab Array: " + tabArray[ci][cj]);
			}
		}
	...
	public void sendRequest() {
		...
		try {
			...
			throw new SoapException("Failed to read the request properly" + ioe.getCause());
		} finally {
	 ...
	 * @version Created: 08/28/2014
	 * @param xpath String: xpath to evaluate
	 * @param value String: Depending on value given, will update the xpath value,
	 *              attribute, or call a separate function. <br>
	 *              <br>
	 *              <b>Value syntax expressions:</b> <br>
	 *              <b>value="fx:funcName"</b> -- Will call the function "funcName"
	 *              to be handled in {@link #handleValueFunction}
	 * @throws XPathExpressionException Could not match evaluate xPath
	 * @throws RuntimeException         Could not match xPath to a node, element or
	 *                                  attribute
	 */
	...
	public void setRequestNodeValueByXPath(Object[][] scenarios) {
		for (int x = 0; x < scenarios.length; x++) {
			logDebug(
					"Set value [ " + scenarios[x][1].toString() + " ] to XPath [ " + scenarios[x][0].toString() + " ]");
	 ...
	 * @param xpath String: xpath to evaluate
	 * @param value String: Depending on value given, will validate the xpath node
	 ...
	 *              <b>value="attribute:attrName,abc"</b> -- Directly states that
	 *              the node attribute "attrName" will be validated and expected to
	 *              be "abc"
	 *
	 */
	...
	protected boolean validateNodeValueByXPath(Document doc, String xpath, String regexValue) {
		...
		if (!errorMessage.isEmpty()) {
			buffer.append("<tr><td style='width: 100px; color: black; text-align: left;'>" + xpath + "</td>");
			buffer.append("<td style='width: 100px; color: black; text-align: center;'>" + regexValue + "</td>");
			buffer.append("<td style='width: 100px; color: black; text-align: center;'>" + errorMessage + "</td>");
			buffer.append("<td style='width: 100px; color: red; text-align: center;'><b>Fail</b></td></tr>");
		} else if (Regex.match(regexValue, xPathValue)) {
			buffer.append("<tr><td style='width: 100px; color: black; text-align: left;'>" + xpath + "</td>");
			buffer.append("<td style='width: 100px; color: black; text-align: center;'>" + regexValue + "</td>");
			buffer.append("<td style='width: 100px; color: black; text-align: center;'>" + xPathValue + "</td>");
	 ...
	 * @param function String: function to call <br>
	 *                 <br>
	 *                 <b>Supported Functions:</b> <br>
	 *                 <b>value="fx:addnode"</b> -- Add a new node at Xpath
	 *                 position. <br>
	 *                 <b>value="fx:getdatetime"</b> -- Set date and time in a
	 *                 format accepted by XML. <br>
	 ...
	 *                 characters. <br>
	 *                 <b>value="fx:randomalphanumeric"</b> -- Set a string of
	...
	private String handleValueFunction(Document doc, String function, String xpath) {
		...
		case "fx:getdatetime":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:getdatetime ; 0] where 0 is Days Out");
			}
		...
		case "fx:getdate":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:getdate ; 0] where 0 is Days Out");
			}
			daysOut = params[1].replace("DaysOut:", "");
		...
		case "fx:addnode":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:addnode ; nodeName] where nodeName is name of the node to add");
			}
		...
		case "fx:addnodes":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:addnodes ; nodeNameList] where nodeNameList is name of the node to add");
			}
		...
		case "fx:addattribute":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:addattribute ; attributeName] where attributeName is name of the attribute to add");
			}
		...
		case "fx:addnamespace":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:addnamespace ; xmlns:name,url] where name is the name of the namespace to add and url is the URL of the namespace");
			}
		...
		case "fx:randomnumber":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:randomnumber ; 4] where 4 is length of numbers to generate");
			}
		...
		case "fx:randomstring":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:randomstring ; 4] where 4 is length of string to generate");
			}
			length = params[1].replace("node:", "").replace("Node:", "");
		...
		case "fx:randomalphanumeric":
			if (params.length == 1) {
				throw new MissingFunctionParameterValueException(
						"Missing second parameter: Expected format [ fx:randomalphanumeric ; 4] where 4 is length of string to generate");
			}
			length = params[1].replace("node:", "").replace("Node:", "");
